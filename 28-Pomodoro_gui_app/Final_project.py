import math
from tkinter import *

# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None

# ---------------------------- TIMER RESET ------------------------------- # 
def reset():
    window.after_cancel(timer)
    canvas.itemconfig(count_text, text='00:00')
    title.config(text='Timer', fg=GREEN)
    tick.config(text='')
    global reps
    reps = 0
# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_timer():
    global reps
    reps += 1
    work_sec = WORK_MIN * 60
    short_break = SHORT_BREAK_MIN * 60
    long_break = LONG_BREAK_MIN * 60
    
    if reps % 8 == 0:
        count_down(long_break)
        title.config(text='Break', fg=RED)
    elif reps % 2 == 0:
        count_down(short_break)
        title.config(text='Break', fg=PINK)
    else:
        count_down(work_sec)
        title.config(text='Work', fg=GREEN)
# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):
    global timer
    count_min = math.floor(count / 60)
    count_sec = count % 60
    if count_sec == 0:
        count_sec = '00'
    elif count < 10:
        count_sec = f'0{count_sec}'
    canvas.itemconfig(count_text, text=f"{count_min}:{count_sec}")
    if count > 0:
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()
        marks = ''
        for rep in range(math.floor(reps/2)):
            marks += '✓'
        tick.config(text=marks)
# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title('Pomodoro')
window.config(padx=100, pady=50, bg=YELLOW)

canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato_img = PhotoImage(file='tomato.png')
title = Label(text='Timer', font=(FONT_NAME, 50, 'normal'), foreground=GREEN, bg=YELLOW)
title.grid(column=1, row=0)
canvas.create_image(100, 112, image=tomato_img)
count_text = canvas.create_text(102, 130, text='00:00', fill='white', font=(FONT_NAME, 30, 'bold'))
canvas.grid(column=1, row=1)

button = Button(text='Start', highlightthickness=0, command=start_timer)
button.grid(column=0, row=2)
button = Button(text='Reset', highlightthickness=0, command=reset)
button.grid(column=2, row=2)
tick = Label(text='', fg=GREEN, bg=YELLOW)
tick.grid(column=1, row=3)

window.mainloop()